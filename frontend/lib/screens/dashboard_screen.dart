...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  void initState() {
    super.initState();
    // Pokrećemo učitavanje vozila pri inicijalizaciji screen-a
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(child: _metricCard('Total vehicles', '$total', Colors.black87)),
              const SizedBox(width: 12),
              Expanded(child: _metricCard('Active', '$active', const Color(0xFF0F6E56))),
              const SizedBox(width: 12),
              Expanded(child: _metricCard('On service', '$maintenance', const Color(0xFF854F0B))),
              const SizedBox(width: 12),
              Expanded(child: _metricCard('Inactive', '$inactive', const Color(0xFFA32D2D))),
            ],
          ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            value,
            style: TextStyle(fontSize: 26, fontWeight: FontWeight.w600, color: valueColor),
          ),
        ],
      ),
    );
  ...
  Widget _statusBar(int active, int maintenance, int outOfService) {
    ...
    if (active > 0) {
      segments.add(Expanded(flex: active, child: Container(color: const Color(0xFF1D9E75))));
    }
    if (maintenance > 0) {
      segments.add(Expanded(flex: maintenance, child: Container(color: const Color(0xFFEF9F27))));
    }
    if (outOfService > 0) {
      segments.add(Expanded(flex: outOfService, child: Container(color: const Color(0xFFE24B4A))));
    }
    if (segments.isEmpty) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Fleet status', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500)),
  ...
  Widget _needsAttentionCard(List<Vehicle> vehicles) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Attention', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500)),
  ...
  Widget _attentionRow(Vehicle v) {
    ...
    final color = isMaintenance ? const Color(0xFF854F0B) : const Color(0xFFA32D2D);
    final bg = isMaintenance ? const Color(0xFFFAEEDA) : const Color(0xFFFCEBEB);
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Icon(isMaintenance ? Icons.build : Icons.warning_amber, size: 18, color: color),
